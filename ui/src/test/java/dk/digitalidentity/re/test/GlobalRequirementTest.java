...
@ActiveProfiles({ "test" })
public class GlobalRequirementTest {
	...
	@Before
	public void before() {
		driver.get(url + "/requirement/listshared");
	}
	...
	public void verifyList() {
		driver.findElement(By.id("sharedTable"));
	}
	...
	public void tryDelete() {
		...
		WebElement listTable = driver.findElement(By.id("sharedTable"));
	...
	public void delete() {
		...
		listTable = driver.findElement(By.id("sharedTable"));
	...
	public void edit() {
		...
		// Loop and find that this element is still on the list with the edited text
	...
	@Test
	public void toBePromoted() {
		addToBePromotedRequirement();
		driver.navigate().refresh();

		WebElement listTable = driver.findElement(By.id("toBePromotedTable"));
		Assert.assertTrue(listTable != null);
	...
	public void promote() {
		...
		// Find element to edit
		Optional<WebElement> toBePromoted = listTable.findElement(By.tagName("tbody")).findElements(By.tagName("tr")).stream().filter(e -> e.findElement(By.tagName("td")).getText().equals(testName)).findAny();
		Assert.assertTrue(toBePromoted.isPresent());

		// Edit
		toBePromoted.get().findElement(By.cssSelector("td:nth-child(4) > a > em.fa-thumbs-o-up")).click();

		// Click Yes, and then OK
		WebDriverUtil.confirmSweetAlert(driver, null);
		WebDriverUtil.confirmSweetAlert(driver, listTable);

		WebElement sharedTable = driver.findElement(By.id("sharedTable"));
		Assert.assertTrue(sharedTable != null);

		Assert.assertTrue(sharedTable.findElement(By.tagName("tbody")).findElements(By.tagName("tr")).stream().anyMatch(e -> e.findElement(By.tagName("td")).getText().equals(testName)));
	}
	...
	public void reject() {
		...
		// Edit
		toBePromoted.get().findElement(By.cssSelector("td:nth-child(4) > a > em.fa-thumbs-o-down")).click();
		...
		Assert.assertFalse(sharedTable.findElement(By.tagName("tbody")).findElements(By.tagName("tr")).stream().anyMatch(e -> e.findElement(By.tagName("td")).getText().equals(testName)));
	}

	private void addToBePromotedRequirement() {
		Requirement requirement = new Requirement();
		requirement.setName(testName);
